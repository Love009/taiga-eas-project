...
static const int    SYSTEMATIC = 0;// =1 => take in account
double dt_syst[N_STN_TOT];
int matchH = 0;
int tmc = 0;//tmc=1 => simulation data => see parameter limits in line 169
...
TH1D *dt_btw_sts_in_evt = new TH1D("dt_btw_sts_in_evt","dt between stations in event",100., 0., 10000.);
TH1D *dt_btw_sts_in_evtTMC = new TH1D("dt_btw_sts_in_evtTMC","dt between stations in event",100., 0., 10000.);
...
void FindTimeMIN_MAX(Event &evt, float *res)
{
    ...
    if(res[1] >= 3000.) more1mks++;
    else less1mks++;
    for(int l = 0; l < evt.NStations() - 1; l++)
    {
        if(tmc != 1) dt_btw_sts_in_evt -> Fill(dtime[l]);
        else dt_btw_sts_in_evtTMC -> Fill(dtime[l]);
       // cout << dtime[l] << "\t";
    }
   // cout << "\n";
...
void FindDirection(Event& evnt)
{
 ...
 /* //delete bad station
    int stNumToDelete = BadStNum(evnt);
    evnt.DeleteStation(stNumToDelete);
  //end of delete bad station*/
    ...
    if(tmc == 1) f1->SetParLimits(2,  -20000/*0.*/, 20000/*1000000000*/);
    else f1->SetParLimits(2,  0., 1000000000);
    f1->SetLineStyle(3);
...
//Writing results
    ...
    evnt.SetPhiRec(phi);
    if(evnt.NStations() >= 4) evnt.SetChi2(chi / (evnt.NStations() - 3.));
...
void Recon(const char *FileName = FILE_TO_RECON, int draw = 1)
{
    ...
    for(int ievt = 0; ievt < (int) evts.size(); ievt++)
    {
        if(evts[ievt].NStations() == 3)
        {
            float timeMIN_MAX[3];
            FindTimeMIN_MAX(evts[ievt], timeMIN_MAX);
        }
    }
    ...
    if(draw) {
        ...
        if(tmc != 1)
        {
            dt_btw_sts_in_evt -> SetLineColor(kBlue);
            dt_btw_sts_in_evt -> Scale(2. / dt_btw_sts_in_evt -> GetEntries());
            dt_btw_sts_in_evt -> GetYaxis() -> SetRangeUser(0.0000001, 1.);
            dt_btw_sts_in_evt -> Draw("SAME");
        }
    }
   // cout << "Noise part = " << more1mks * 1. / (/*more1mks * 1. + */less1mks * 1.) * 100. << " percent";//????
...
int main1()
{
    ...
    less1mks = 0;
    more1mks = 0;
    Recon("test_plane5pers.root", 1);
    c0001 -> cd(1);
    dt_btw_sts_in_evtTMC -> SetLineColor(kRed);
    dt_btw_sts_in_evtTMC -> Scale(2. / dt_btw_sts_in_evtTMC -> GetEntries());
    dt_btw_sts_in_evtTMC -> Draw("HIST SAME");

    double noise_ppart = dt_btw_sts_in_evt -> Integral(30, 100) / 0.7 * 2 * 100.;
    double noise_ppartTMC = dt_btw_sts_in_evtTMC -> Integral(30, 100) / 0.7 * 2 * 100.;
    cout << "Noise part: data = " << noise_ppart << "\tTMC = " << noise_ppartTMC << endl;
    return 0;
}
