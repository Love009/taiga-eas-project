...
int Generate(int Ngen = 1000, const char* fileToSave = FILE_NAME)
{
    ...
    for (int i = 0; i < Ngen; i++)
    {
        ...
        event->SetPhiGen(a);
        event->SetThetaGen(b);
       // points -> Fill(180. / TMath::Pi() * a * TMath::Sin(HiSC_the), 180. / TMath::Pi() * b);
        ...
        for (int j = 0; j < N_STN_TOT; j++) {
            ...
            if ((gRandom -> Rndm()) < P_STN_TRG * exp(-R/R0)) {
                ...
                double dr[3];
                dr[0] = SPH_FRONT_RADIUS * cos_gen[0] + (MuonX[j] - x_coord);
                dr[1] = SPH_FRONT_RADIUS * cos_gen[1] + (MuonY[j] - y_coord);
                dr[2] = SPH_FRONT_RADIUS * cos_gen[2];// + MuonZ[j];
                double timeGen = sqrt(dr[0] * dr[0] + dr[1] * dr[1] + dr[2] * dr[2]) / 0.3 - SPH_FRONT_RADIUS / 0.3 + MuonTerr[j] *(r2->Gaus(0.,1.));
        ...
        //Noise generation
        //if(i > Ngen * 0.46/*0.87*/ && nstn >= 2)
        if(i > Ngen * 0.94 && nstn == 2)
        {
            int flag = 0;
            int id_noise;
            do{
        ...
        if(nstn >= 3)
        {
            ...
            event -> SetEAScenterGen(coordinates);
    ...
    points -> Draw("CONT4Z");
    cout << "\tNnoise part = " << counterNoise * 1. /  (counterSignal * 1.) * 100. << endl;
    return 0;
}
//======================================================================================================================
...
void FillHists(const char *FileName, int draw = 1)
{
 ...
 int DrawCompare()
 {
    ...
    FillHists("analysis_results/for_EAS_center/Gr+Hisc241119ReconRecon.root", 0);
    FillHists("analysis_results/for_EAS_center/Gr+Hisc241219ReconRecon.root", 0);
    FillHists("analysis_results/for_EAS_center/Gr+Hisc291219ReconRecon.root", 0);
    FillHists("analysis_results/for_EAS_center/Gr+Hisc020120ReconRecon.root", 0);
   ...
   // FillHists("analysis_results/for_EAS_center/Gr+Hisc250220Recon.root", 0);
  //  FillHists("analysis_results/for_EAS_center/Gr+Hisc270220Recon.root", 0);
   // FillHists("analysis_results/for_EAS_center/Gr+Hisc210220Recon.root", 1);
    return 0;
 }